...
### Reference File Loading（必須・タスク着手時）

> sprint-workflow.mdc（Core）はセッション判定・ライフサイクル・ガードのみ含む。
> タスク実行フェーズの詳細仕様は以下のReference Fileに含まれる。

**タスク着手時に必ず以下を Read する:**
- `~/.cursor/references/workflow/execution-phase.md` — タスク管理手順、Wave並列実行、スコープ変更ルール、仕様遵守チェック
...
- [ ] milestones.md / tasks.md 更新時、3箇所（YAML集計値・進捗テーブル・個別セクションテキスト）が整合しているか ★TRY-063
...
| times-agent | タスク完了時 | Slack分報投稿（member_name: sprint-documenter）。Cursor: times-agentサブエージェント起動。Claude Code: sprint-masterに委任（Step 6d） |
