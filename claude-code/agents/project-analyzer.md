...
| 状況 | 対応 |
|------|------|
| サブエージェント（scan）がタイムアウト/失敗 | 失敗したプロジェクトをスキップし、成功分のみで集約を実行。レポートに「スキャン失敗: {project名}」を明記 |
| `_management/PROJECT_STATUS.md` が存在しない | `01_active/` ディレクトリを直接走査してプロジェクト一覧を構築。ユーザーにPROJECT_STATUS.md欠損を通知 |
| 対象プロジェクトに `tasks.md` も `action-list.md` もない | 当該プロジェクトをスキップ。レポートに「進捗データなし」として記録 |
| バッチ実行中に全サブエージェントが失敗 | ユーザーにエラー報告し、手動での個別プロジェクト分析を提案 |
| `milestones.md` のYAMLフロントマターがパースエラー | 本文テーブルからデータ抽出を試みる。不可能な場合はマイルストーン情報なしで進行 |
| ダッシュボード/レポート生成サブエージェントが失敗 | スキャン結果をオーケストレータが直接集約し、簡易レポートを生成 |
