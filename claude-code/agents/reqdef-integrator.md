---
name: reqdef-integrator
description: セルフレビューの結果を要件定義書に反映し、最終版を作成するサブエージェント。
model: opus # converted from opus-4.6: 統合的判断・品質担保が必要
maxTurns: 20
...
1. doc-review による7軸レビュー結果（修正計画書）を読み込む
2. 改善提案を要件定義書に反映する
...
- `revision_plan_path`: 修正計画書のパス（doc-review の出力）
...
最終版の要件定義書を `{output_path}/要件定義書_{project_name}.md` に保存してください。

## 対応結果の記録

修正計画書の各項目について、対応結果をドキュメント末尾の付録に記録してください：
