...
### Reference File Loading（必須・最初に実行）

> sprint-workflow.mdc（Core）はセッション判定・ライフサイクル・ガードのみ含む。
> プランニングフェーズの詳細仕様は以下のReference Fileに含まれる。

**フェーズ開始時に必ず以下を Read する:**
- `~/.cursor/references/workflow/planning-phase.md` — プロダクトバックログ、リファインメント（ToT含む）、プランニング手順、CW-Awareプランニング
...
|--------|------|-------------------------|---------|
| **委譲モード** | Step 0〜6をTask toolサブエージェントに委譲し、Step 7-8をメインで実行 | 低（15-30K tokens推定） | **デフォルト（全ティアで有効）** |
| **従来モード** | メインエージェントがStep 0〜8を逐次実行 | 高（83-166K tokens推定） | POが明示的に「従来モードで」と指定した場合のみ |

### モード判定ロジック

> **変更（SPRINT-041 / PBI-029）**: 全ティアで委譲モードをデフォルト化。CW最適化Phase 1+2（PBI-024〜028）で入力ファイルの軽量化が完了し、委譲モードの実用性が全ティアで確保されたため。
...
[モード判定]
    │
    ├── POが「従来モードで」と明示指定 → 従来モード
    │
    └── それ以外（デフォルト）→ 委譲モード【全ティア共通】
         └── サブエージェント起動失敗 → 従来モードにフォールバック
    │
    ▼
選択されたモードでプランニング実行
```
...
- POが「従来モードで」と明示 → 従来モードに切替
- 委譲モードでサブエージェント起動失敗 → 従来モードにフォールバック

> **注**: 委譲モードがデフォルトのため、「委譲モードで」の明示指定は不要。従来モードへの切替のみがオーバーライド対象。
...
> CW消費を最小化するため、各入力ファイルは必要部分のみを読み込む。
> 全文読み込みは「深い分析が必要」と判断した場合のみ実施する。

| ファイル | 読み込み範囲 | 読み込み方法 | 全文読み込みトリガー |
|---------|------------|------------|-------------------|
| `scrum/tasks.md` | YAMLフロントマター + 進捗サマリー + 未完了タスクセクション | Read offset=1, limit=60。未完了タスクがある場合は該当Phase セクションを追加Read | 依存関係の詳細分析が必要な場合 |
| `scrum/kpt-history.md` | YAMLフロントマター + 直近3スプリント分のKPTエントリ | Read offset=1, limit=20（YAML+サマリー）。直近KPTはファイル末尾付近なので Read offset=-100 | Problem パターン分析が必要な場合 |
| `scrum/team-roster.md` | YAMLフロントマター + メンバー一覧テーブル | Read offset=1, limit=60（YAML+メンバー一覧。稼働履歴はスキップ） | メンバーの稼働履歴を確認する必要がある場合 |
| `~/.cursor/try-stock.md` | YAMLフロントマター + Pendingセクション（ai-scrum-framework の High/Medium のみ） | Read offset=1, limit=20（YAML+集計）。Pendingセクションは Grep で "### ai-scrum-framework" を検索して該当セクションのみRead | 他プロジェクトのTryも確認する場合 |
| `scrum/product-backlog.md` | 全文（通常サイズが小さいため） | Read（全文） | — |
| `scrum/milestones.md` | YAMLフロントマター + 進捗サマリーテーブル | Read offset=1, limit=50 | マイルストーン詳細を確認する場合 |

**原則**:
1. まずYAMLフロントマター（先頭~20行）を読み、全体像を把握する
2. 必要なセクションのみを追加で読み込む
3. 「全文が必要」と判断した場合のみ全文Read（判断理由をログに記録）
...
プランニング開始（sprint-masterから委譲）
    │
    ▼
[モード判定] ★ UPDATED (SPRINT-041 / PBI-029)
    │ デフォルト: 委譲モード（全ティア共通）
    │ → 委譲モード（デフォルト）: 「サブエージェント委譲モード」セクションへ遷移（Step 0〜6を委譲）
    │ → 従来モード（PO明示指定時のみ）: 以下の Step 0〜8 を逐次実行
    │
    ▼
...
[Step 1: リファインメント要否の判断]
    ...
    ▼
[Step 2: データ収集・分析]
    │ ※ 入力ファイルの読み込みは「選択的読み込みプロトコル」に従う（CW最適化）
...
[Step 5: バックログ組成]
    ...
    │
    ▼
[Step 5.3: メンバー視点の集約] ★ PBI-015 / PBI-027
    │ バックログ案に対して、各Skill定義の専門的視点からフィードバックを収集する。
    │ レトロスペクティブの「メンバー視点の集約」と同じパターンを適用。
    │
    │ ★ CW最適化: Skill全文(~74KB)を読み込む代わりに、集約ファイルを参照する:
    │   Read: ~/.cursor/references/workflow/planning-perspectives.md (~5KB)
    │   ※ 8 Skill視点（coder/documenter/researcher/mentor/reviewer/refactorer/po-assistant/sp-estimator）を集約済み
    │
    │ 集約ルール:
...
★ CW最適化（PBI-028）: 直近3ログ全文(~21KB)の代わりにサマリーファイルを優先参照する。

**読み込み優先順位:**
1. `{project_root}/.sprint-logs/velocity-summary.md` が存在する場合 → このファイルを読み込む（~2KB）
2. 存在しない場合（フォールバック）→ `.sprint-logs/SPRINT-*.md` 直近3件のYAMLフロントマターのみ読み込む
...
- **委譲モード（デフォルト）**: サブエージェントの返却結果の妥当性をメインエージェントが検証してからPOに提示すること。全ティアで委譲モードがデフォルト（SPRINT-041 / PBI-029）

---

## サブエージェント委譲モード（CW最適化）

> **導入**: SPRINT-033 (PBI-017+018)、**デフォルト化**: SPRINT-041 (PBI-029)
> **設計書**: docs/analysis/planning-context-optimization.md
> **目的**: プランニング分析フェーズ（Step 0〜6）をTask toolサブエージェントに委譲し、メインセッションのCW消費を約82%削減する
> **ステータス**: 全ティア（S/M/L）でデフォルト有効。POが「従来モードで」と明示した場合のみ従来モードに切替
...
### Step 2: データ収集・分析
※ 入力ファイルの読み込みは「選択的読み込みプロトコル」に従う（本ドキュメントの「入力ファイル」セクション参照）
- milestones.md → 進行中マイルストーンの期限・進捗率
- tasks.md → 未着手タスク（⬜）を全件抽出
- try-stock.md → 優先度HighのPending Tryを抽出
- ★ ベロシティ参照（CW最適化）: まず {project_root}/.sprint-logs/velocity-summary.md を読み込む（~2KB）。存在しない場合のみ .sprint-logs/SPRINT-*.md 直近3件のYAMLフロントマターにフォールバック
...
### Step 5.3: メンバー視点集約
★ CW最適化（PBI-027）: Skill全文(~74KB)の代わりに集約ファイルを参照:
- Read: ~/.cursor/references/workflow/planning-perspectives.md (~5KB, 8 Skill視点集約)
各視点からフィードバックを収集し「合意」「懸念」「提案」で整理。
...
sprint-backlog.md のYAMLフロントマターに以下を記録する:

```yaml
sprint:
  # ... 既存フィールド ...
  model_tier: "S | M | L"           # モデルティア判定結果
  planning_delegation: true          # 委譲モードで実行（デフォルト true）
```

> **SPRINT-041 / PBI-029**: `planning_delegation` のデフォルト値が `true` に変更。従来モードを使用した場合のみ `false` を明示記録する。
