...
### Reference File Loading（必須・レビュー開始時）

> sprint-workflow.mdc（Core）はセッション判定・ライフサイクル・ガードのみ含む。
> レビュー・レトロフェーズの詳細仕様は以下のReference Fileに含まれる。

**レビュー開始時に必ず以下を Read する:**
- `~/.cursor/references/workflow/review-retro-phase.md` — レビュー手順、POフィードバック処理、レビュー→レトロ遷移ルール
...
[Step 3: PO共有+フィードバック確認]
    ...
    │
    │ ★★★ PO応答待機（SPRINT-042 / TRY-085）★★★
    │ 閉鎖型質問を出したら、**必ずここでターンを終了する**。
    │ POの応答メッセージを受け取るまで、Step 4以降の処理を開始してはならない。
    │ - ❌ 質問と同じターンでレトロスペクティブに遷移すること
    │ - ❌ PO応答を仮定して先に進むこと
    │ - ✅ 質問後にターンを終了し、POの次のメッセージで処理を再開する
    │
    │ ★ PO応答パターン（即座判定）:
...
| POがフィードバックなしで終了（「OK」「なし」等） | フィードバックなしとして記録し、レビュー完了。**即座にレトロスペクティブに自動遷移する**（PO再確認不要）。ただし閉鎖型質問後のPO応答受信は必須（TRY-085） |
