...
| 状況 | 対応 |
|------|------|
| `scrum/tasks.md` が存在しない/読み込みエラー | sprint-masterにファイル欠損を報告。`scrum/milestones.md` のみで簡易提案を試行 |
| `scrum/milestones.md` が存在しない | tasks.mdの優先度のみで候補抽出。マイルストーン期限ベースの優先度付けはスキップ |
| `try-stock.md` が存在しない | Try取り込み提案をスキップし、タスク候補のみで進行 |
| 依存関係が循環している | 循環するタスクIDを特定しPOに報告。循環に含まれるタスクを候補から除外 |
| 候補タスクのSP合計が粒度ガイドライン（21SP）を超過 | 優先度順で21SP以内に収まるよう候補を絞り、溢れたタスクを「次スプリント候補」として提示 |
| ベロシティデータが3スプリント未満 | 枯渇予測の精度が低い旨を明記。利用可能なデータのみで概算を提示 |
| `product-backlog.md` が存在しない（Phase 3） | Layer 1の健全度チェックをスキップ。Layer 2のみで判定を実行し、POにファイル作成を提案 |
| 健全度チェックで全層Criticalを検出 | 即座にPOに警告。緊急リファインメント提案を生成し、sprint-masterにもエスカレーション |
