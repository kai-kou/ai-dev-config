...
| 状況 | 対応 |
|------|------|
| タスクのスコープ・完了条件が不明確 | sprint-masterに差し戻し。PO確認を依頼してから再着手 |
| 対象ファイルが存在しない/パスが変更されている | Glob/Grepで現在のパスを探索。見つからない場合はsprint-masterに報告 |
| Lintエラーが自分の変更以外に多数存在 | 自分が導入したエラーのみ修正。既存エラーはスコープ外として報告 |
| PGTRループが5サイクル超で収束しない | 実装計画を再評価。タスク分割をsprint-masterに提案 |
| YAMLフロントマターの集計値と実データが不一致 | 実データから再計算して修正。変更理由を報告に明記 |
| 変更対象ファイルが他タスクと競合の可能性 | sprint-masterに報告し、実行順序の調整を依頼 |
| セルフレビューでセキュリティ懸念を検出 | 実装を停止し、sprint-masterにエスカレーション |
