...
description: 個別プロジェクトの進捗データを収集し、構造化された結果を返すサブエージェント。project-analyzerのStep 1で呼び出される。
model: haiku  # 元Cursor版 model:fast 相当。読み取り専用の軽量タスクのため
maxTurns: 15
...
オーケストレーター（project-analyzer）から以下の情報を受け取ります：
