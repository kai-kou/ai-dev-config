...
### Reference File Loading（必須・レトロ開始時）

> sprint-workflow.mdc（Core）はセッション判定・ライフサイクル・ガードのみ含む。
> レトロスペクティブの詳細仕様は以下のReference Fileに含まれる。

**レトロ開始時に必ず以下を Read する:**
- `~/.cursor/references/workflow/review-retro-phase.md` — KPT整理、ログ確定、sprint-backlog.mdステータス更新、アーカイブ保存

このファイルを読み込んでから、以下のStepに進むこと。
...
[Step 2: KPT整理]
    ...
    │ Try: 具体的な改善提案（1〜3項目）
    │
    │ ★ CW最適化振り返りチェック（SPRINT-041〜 / PBI-029）:
    │   sprint-backlog.md の planning_delegation フィールドを確認し、
    │   以下の観点をKPTに反映する（該当する場合のみ）:
    │   ・委譲モードが正常に動作したか（Keep候補）
    │   ・フォールバックが発生したか（Problem候補 + 原因分析）
    │   ・プランニングのCW消費が期待値（≤15%）に収まったか
    │   ・委譲プロンプトテンプレートの改善点はあるか（Try候補）
    │
    ▼
[Step 3: メンバー視点の集約]
...
[Step 5: PO効率指標の計測] ★ SPRINT-030 強化（T110）
    ...
    │
    │
    │ ■ 指標6: Observability Metrics ★ SPRINT-035 追加（T401）
    │   sprint-backlog.md と sprint実行ログから以下を収集し、
    │   sprint-log.md Section 7 + YAMLフロントマター observability に記録する。
    │
    │   (a) 実行環境:
    │     ・model_tier: sprint-backlog.md の YAML sprint.model_tier から転記
    │     ・planning_delegation: sprint-backlog.md の YAML sprint.planning_delegation から転記
    │     ・execution_mode: sprint-backlog.md の YAML sprint.execution_mode から転記
    │     ・autonomous: sprint-backlog.md の YAML sprint.autonomous から転記
    │
    │   (b) CW予算配分（概算%）:
    │     正確な計測は困難なため、以下の概算ガイドで判断する:
    │     ・プランニングが会話の1/4以上を占めた → planning_pct: 25+
    │     ・タスク実行が会話の大半を占めた → execution_pct: 70+
    │     ・レビュー/レトロが会話の1/4以上を占めた → review_retro_pct: 25+
    │     ・3フェーズ合計が100%になるよう概算を調整する
    │     ・委譲モード使用時: プランニング比率 ≤ 15% が期待値
    │     ・★ SPRINT-041〜（PBI-029）: 委譲モードがデフォルト化。以下を追加記録:
    │       - planning_delegation_effective: boolean（委譲モードが実際に使用されたか）
    │       - delegation_fallback: boolean（従来モードにフォールバックしたか）
    │       - cw_savings_note: string（CW削減効果の定性的メモ。例:「プランニング15%以内に収まった」等）
    │
    │   (c) Slack投稿サマリー:
    │     sprint-backlog.md の「Slack投稿記録」テーブルから集計する
    │     ・total: テーブル行数（SM完了投稿含む）
    │     ・success_subagent: 投稿手段が「サブエージェント」の成功数
    │     ・success_mcp: 投稿手段が「MCP直接」の成功数
    │     ・fallback_chat: 投稿手段が「チャット内記録」の数
    │     ・skipped: スキップ数
    │
    ▼
[Step 5.5: SP精度振り返り]
...
[Step 7: Try登録]
    ...
    │
    ▼
[Step 7.1: try-stock YAML集計値照合] ★ SPRINT-035 追加（TRY-071）
    │ ※ Step 7でtry-stock.mdを更新した後に必ず実行する。
    │ ※ YAML集計値とテーブル実データの乖離を構造的に防止する。
    │
    │ (1) try-stock.md の YAMLフロントマターから集計値を取得:
    │   ・pending: {N}
    │   ・in_progress: {N}
    │   ・completed: {N}
    │   ・cancelled: {N}
    │   ・total: {N}
    │
    │ (2) try-stock.md の各セクション（Pending/In Progress/Completed/Cancelled）
    │   のテーブル行数を実際にカウントする:
    │   ・Pending実数: Pendingセクションのテーブルデータ行数（ヘッダ・区切り行を除く）
    │   ・In Progress実数: 同上
    │   ・Completed実数: 同上
    │   ・Cancelled実数: 同上
    │   ・Total実数: 上記4セクションの合計
    ...
    │
    │ (4) 不一致がある場合:
    │   ・YAML値を実数に合わせて修正する（実テーブルが正）
    │   ・修正内容をレトロログに記録:
    │     「try-stock YAML照合: {フィールド} YAML={旧値}→実数={新値} に修正」
    │   ・Problemとして記録するかは乖離の程度で判断:
    │     1件の乖離 → 修正のみ（Problem不要）
    │     複数フィールドの乖離 → 軽微Problemとして記録
    │
    │ (5) 全フィールド一致の場合:
    │   レトロログに「try-stock YAML照合: 全フィールド一致 ✅」と記録
    │
    ▼
[Step 7.3: Medium Try自動降格チェック] ★ SPRINT-034 追加（TRY-060）
    │ ※ 毎スプリント実行（トリガー条件なし・軽量チェック）
    │
    │ try-stock.md の Pending セクションを走査し、以下の条件に該当する
    │ Tryを自動的に降格する:
    │
    │ | 現在の優先度 | 条件 | アクション |
    │ |------------|------|------|
    │ | Medium | 登録から5スプリント以上経過し未着手 | → Low に降格 |
    │ | High | 登録から3スプリント以上経過し未着手（POが意図的に保留でない限り） | → Medium に降格 |
    │
    │ ■ 実行手順:
    │ (1) Pending の Medium/High Try をリスト化
    │ (2) 各Tryの登録スプリント番号と現在のスプリント番号の差を計算
    │ (3) 条件に該当するTryの優先度を変更
    │ (4) 変更があった場合、try-stock.md を更新し、
    │     レトロログに「自動降格: {N}件（{TRY-IDs}）」と記録
    │
    │ ※ このステップは Step 7.5（棚卸し）の降格判定と同じロジックだが、
    │   棚卸しトリガーを待たずに毎スプリント実行することで
    │   Pending件数の自然な圧縮（優先度降格による実質的な優先度整理）を実現する。
    │ ※ Step 7.5 の降格判定 (3) は、Step 7.3 で既に降格済みのTryを二重処理しない。
    │
    ▼
...
[Step 8.5: KPT履歴集約]
    ...
    │
    ▼
[Step 8.6: tasks.md 重点タスク更新] ★ SPRINT-032追加（TRY-064）
    │ scrum/tasks.md の「今週の重点タスク」セクションを更新する
    │ ※ このセクションは次スプリント候補の可視化を目的とする
    │ ※ レトロで定常的に更新しないと陳腐化する（SPRINT-029〜031で連続発生）
    │
    │ (1) 現在の「今週の重点タスク」セクションの内容を確認
    │ (2) 以下の情報源から次スプリント候補を抽出:
    │   ・scrum/tasks.md の未完了タスク（⬜ / 🔄）
    │   ・scrum/milestones.md の進行中マイルストーンの残タスク
    │   ・try-stock.md の High 優先度 Pending Try
    │ (3) 優先度・依存関係・マイルストーン期限を考慮して上位3〜5件を選定
    │ (4) テーブルを更新:
    │   | ID | タスク | 優先度 | マイルストーン | ステータス |
    │   ※ 完了済みタスクはテーブルから除外
    │   ※ 全タスク完了の場合は「全タスク完了」と記載
    │ (5) テーブル上部の説明文（> で始まるブロック引用）も現状に合わせて更新
    │
    ▼
[Step 8.7: メンバー状態更新]
    ...
    │
    ▼
[Step 8.75: ログ生成バリデーション] ★ SPRINT-008追加（T501）
    │ ※ sprint-backlog.md を completed に更新する前に、
    │ ※ スプリントログファイルの存在を必ず検証する。
    │ ※ 背景: SPRINT-003〜007でログファイルが生成されずに
    │ ※ sprint-backlog.md だけ completed になる問題が発生した。
    │
    │ (1) {project_root}/.sprint-logs/SPRINT-{ID}.md の存在を確認する
    │   ・{ID} は sprint-backlog.md の YAML sprint.id から取得
    │
    │ (2) 存在しない場合:
    │   ・⚠️ ログを出力: 「SPRINT-{ID}.md が存在しません。Step 8 を再実行します。」
    │   ・Step 8（スプリントログ最終化）に戻り、ログを生成する
    │   ・生成後、再度存在を確認する
    │   ・それでも存在しない場合は、エラーとしてPOに報告する:
    │     「⚠️ SPRINT-{ID}.md の生成に失敗しました。手動で確認してください。」
    │   ・★ sprint-backlog.md を completed に更新しない（Step 8.8 をスキップ）
    │   ・★ Step 8.9（アーカイブ）も実行しない
    │
    │ (3) 存在する場合:
    │   ・「✅ ログ生成バリデーション: SPRINT-{ID}.md 確認OK」とログに記録
    │   ・Step 8.8 に進む
    │
    ▼
[Step 8.8: sprint-backlog.md ステータス更新] ★ SPRINT-026追加
    ...
    │ ※ ★ Step 8.75 でログ存在が確認済みの場合のみ実行する（SPRINT-008追加）
    │
    ▼
[Step 8.9: sprint-backlog.md アーカイブ保存] ★ SPRINT-027追加
    ...
    │
    ▼
[Step 8.10: velocity-summary.md 自動更新] ★ SPRINT-040追加（PBI-028）
    │ 直近3スプリントのベロシティサマリーを自動更新する。
    │ ※ sprint-plannerが直近3ログ全文(~21KB)を読み込む代わりに、このサマリー(~2KB)を参照する。
    │
    │ (1) 対象スプリントの特定:
    │   {project_root}/.sprint-logs/SPRINT-*.md をスキャンし、
    │   最新のcompletedスプリント3件を取得（現スプリント含む）
    │
    │ (2) 各スプリントログからYAMLフロントマターを読み取る:
    │   各ファイルの先頭30-40行（YAMLフロントマター部分のみ）をRead
    │   取得フィールド:
    │     - sprint.id, sprint.date
    │     - metrics.planned_sp, metrics.completed_sp, metrics.sp_completion_rate
    │     - metrics.po_wait_time_minutes（オプション）
    │     - metrics.autonomous_rate（オプション）
    │     - metrics.session_effective_rate（オプション）
    │
    │ (3) velocity-summary.md の生成/更新:
    │   ファイルパス: {project_root}/.sprint-logs/velocity-summary.md
    │   テンプレート: ~/.cursor/templates/velocity-summary.md を参照
    │   YAMLフロントマター + Markdownテーブルの両方を更新
    │
    │ (4) 集計値の計算:
    │   - avg_planned_sp: 3スプリントの planned_sp の平均（小数第1位まで）
    │   - avg_completed_sp: 3スプリントの completed_sp の平均（小数第1位まで）
    │   - avg_completion_rate: 3スプリントの sp_completion_rate の平均（整数%）
    │   - velocity_trend: completed_sp の推移から判定
    │     increasing: 直近が前回より多い
    │     stable: ±20%以内で安定
    │     decreasing: 直近が前回より少ない
    │
    │ (5) ファイル保存:
    │   velocity-summary.md を {project_root}/.sprint-logs/ に保存
    │   既存ファイルがある場合は上書き更新
    │
    ▼
[Step 9: 完了報告]
...
- Try登録後のtry-stock.mdのYAMLフロントマター集計値を必ず更新する（Step 7.1で照合検証を実施）
