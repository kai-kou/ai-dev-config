...
**生成ルール**:
- 人格設定の口調・性格・トーンに厳密に従う
- Slack mrkdwn記法を活用: `*太字*`, `_斜体_`, `:emoji:`
- 堅すぎず、分報らしいカジュアルさを保つ
- 100〜300文字程度。長すぎるとタイムラインを圧迫する
...
slack-fast-mcp MCPサーバーの `slack_post_message` ツールで投稿：

- **channel**: 入力パラメータの `channel`、または人格設定の `default_channel`
- **message**: Step 2 で生成した投稿文

**重要**: チャンネル指定は**チャンネルID**（例: `C0AE6RT9NG4`）を使用すること。チャンネル名では `channel_not_found` エラーになる。

**フォールバック（MCP未利用時）**:

`slack_post_message` ツールが利用できない場合、Shell経由でcurlで直接投稿：
